...
describe('db.ts', () => {
  ...
  describe('migrateDb', () => {
    ...
    it('applies migrations #2 and #3 when migration #1 is already done', async () => {
      ...
      expect(mockSql).toHaveBeenCalledTimes(685);
      // Migrations #3 and #15-193 each call sql.transaction() once for atomic execution
      expect(mockSql.transaction).toHaveBeenCalledTimes(180);
    });
